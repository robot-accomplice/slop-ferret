...
//
//	slop plan   <magma-map-dir> <pinned-sha> <repo> [--since <ref>]  > plan.json
...
const instructions = "Read every h_required path — that tier is the floor and an unattested one " +
	...
	"candidates_refuted:[{file,symbol}]} and run `slop verify`. `coverage_waived` entries may be " +
